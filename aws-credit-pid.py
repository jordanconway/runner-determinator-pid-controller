...
class AWSCreditController:
    ...
    def get_current_spend(self, project_id="391835788720"):
        ...
        tenant_id = 'cc951ada-105f-40b1-8305-c65861490a90'
        url = f'https://api.ternary.app/analytics/query/load?tenant_id={tenant_id}'
        
        headers = {
            'Authorization': ternary_api_key,
            ...
            if "response" in response_json and isinstance(response_json["response"], list):
                if response_json["response"]:
                    ...
                    if "credits" in first_item:
                        # Return the absolute value of credits as a positive number
        ...
        try:
            ...
            if "response" in response_json and isinstance(response_json["response"], list):
                if response_json["response"]:
                    first_item = response_json["response"][0]
                    if "credits" in first_item:
                        # Get the absolute value of credits for the past 24 hours
                        credits_24h = abs(first_item["credits"])
                        # Convert to daily rate (multiply by 24/24 = 1, but kept for clarity)
                        daily_rate = credits_24h * (24/24)
                        return daily_rate
                    else:
                        raise ValueError("Response does not contain credits data")
                else:
